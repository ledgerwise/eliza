...
The Core Package (`@ledgerwise/elizaos-core`) provides the fundamental building blocks of Eliza's architecture, handling essential functionalities like:
...
```bash
pnpm add @ledgerwise/elizaos-core
```
...
```typescript
import { AgentRuntime } from "@ledgerwise/elizaos-core";
